#!/usr/bin/env python3
"""
SKYN v10 — SEUIL MACRO RENFORCÉ + COOLDOWN RÉDUIT
===================================================
Objectif : éliminer les pertes de fin de tendance, cibler +200% en 2 mois.

Leçons v9 :
  ✗ Apr-Jun : 4 pertes même en "bear" macro → seuil bear trop permissif (1%)
  ✓ 3F WR = 41% stable  ✓ 4F exclu  ✓ neutral skip

Nouveautés vs v9 :
  - Macro RENFORCÉ : bull si ema50 > ema200×1.015 (était 1.01)
                     bear si ema50 < ema200×0.985 (était 0.99)
    → Neutral élargi : transitions de marché mieux filtrées
  - Cooldown 4h → 2h : re-entrée plus rapide après loss
  - Adaptive sq_bars : si macro "STRONG" (>2% divergence), sq_bars = max(3, sq_bars-2)
    → Plus d'opportunités dans les tendances claires
  - Macro "strong" tracker : affiche jours en tendance forte
"""
...
BO_COOLDOWN_H     = 2   # v10 : 2h (était 4h) — re-entrée plus rapide
COMMISSION        = 0.0004
SLIPPAGE          = 0.0005
...
def _build_btc_macro(btc_sd: dict) -> Dict[pd.Timestamp, str]:
    """
    Retourne date → "bull"|"bear"|"bull_strong"|"bear_strong"|"neutral".
    Seuils v10 renforcés (±1.5% au lieu de ±1%) pour réduire les faux signaux.
    "strong" = divergence ema50/ema200 > 2% — utilise sq_bars allégé.
    """
    ...
    for ts, row in df_d.iterrows():
        try:
            ...
            ratio = e50 / e200
            if ratio > 1.02:
                result[ts.date()] = "bull_strong"
            elif ratio > 1.015:
                result[ts.date()] = "bull"
            elif ratio < 0.98:
                result[ts.date()] = "bear_strong"
            elif ratio < 0.985:
                result[ts.date()] = "bear"
            else:
                result[ts.date()] = "neutral"
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
    ...
    for ts in common_timestamps:
        ...
        macro_dir = macro.replace("_strong", "")  # "bull" or "bear" or "neutral"
        macro_strong = macro.endswith("_strong")
        ...
        # v10 : skip macro neutral (seuil ±1.5% renforcé)
        if macro_dir == "neutral":
            continue

        # En macro "strong", sq_bars allégé pour capter plus d'opportunités
        eff_sq_bars = max(3, sq_bars - 2) if macro_strong else sq_bars
        ...
        for sym, sd in sym_data.items():
            ...
            nf, action = _check_breakout(sd, bar, adx_i, sd["adx_s"], eff_sq_bars, score_min)

            # v9/v10 : seulement 3F et 5F (4F exclu)
            if nf not in ALLOWED_FILTER_COUNTS or action is None:
                continue

            # Filtre macro BTC (bull_strong et bear_strong → direction stricte)
            if macro_dir == "bull" and action == "SELL": continue
            if macro_dir == "bear" and action == "BUY":  continue

            dir_sc = int(sd["buy_sc"][bar]) if action == "BUY" else int(sd["sell_sc"][bar])
            candidates.append({"score": dir_sc, "filters": nf, "sym": sym,
                                "action": action, "bar": bar, "strong": macro_strong})
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v10 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v10[/bold cyan]",
        border_style="cyan",
    ))
    ...
    console.rule(f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades · 4F exclus · macro ±1.5%[/bold]")
...
def run_configs(sym_data, window_timestamps, btc_macro, window_start):
    ...
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"(3F+5F, no neutral ±1.5%, strong→sq-2, {N_MAX_POSITIONS} pos, cd={BO_COOLDOWN_H}h)...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v10 — MACRO ±1.5% / ADAPTIVE SQ / COOLDOWN 2H[/bold yellow]")
    ...
    # Afficher la répartition macro sur la période
    macro_counts: dict = {}
    for v in btc_macro.values():
        macro_counts[v] = macro_counts.get(v, 0) + 1
    bs = macro_counts.get("bull_strong", 0); b = macro_counts.get("bull", 0)
    rs = macro_counts.get("bear_strong", 0); r = macro_counts.get("bear", 0)
    n  = macro_counts.get("neutral", 0)
    console.print(f"\n  Macro BTC (2 ans) : "
                  f"bull_strong={bs}j  bull={b}j  "
                  f"bear_strong={rs}j  bear={r}j  "
                  f"neutral={n}j  "
                  f"[dim](trading actif : {bs+b+rs+r}j / {bs+b+rs+r+n}j)[/dim]")
    ...
    console.print()
    cmp_table = Table(title="Comparatif v10 — 3 Configurations (macro ±1.5%, strong→sq-2, 4F exclu)",
