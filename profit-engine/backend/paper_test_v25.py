#!/usr/bin/env python3
"""
PRISM v25 — BB Squeeze | ATR Leverage | Fenêtre fixe
======================================================
v25 = v22 + une seule amélioration : levier ATR percentile (8-20×).

Diagnostic v23/v24 → causes des échecs :
  1. v23 : mécanisme streak anti-optimal pour WR < 50%
     ARM donnait plus de risque aux SL qu'aux TP (pervers)
  2. v24 : cache expiré → données commencent Dec 21 (266 barres de plus)
     Décalage de 11 jours dans les signaux → résultats incomparables à v22

v25 correctifs :
  • Fenêtre fixe 2026-01-01 → 2026-06-20 (même période que v22)
  • Levier ATR percentile : élevé en compression (squeeze), faible en vol
    → 20× quand ATR percentile = 0 (marché calme, ideal Pattern C)
    → 8×  quand ATR percentile = 1 (marché volatile)
  • Risque & dd_scale identiques v22 (pas d'ARM streak)
  • Score ≥ 80 : +2 levier (bonus signal fort, même logique)

Attendu vs v22 (+58.1%, MaxDD 23.2%, Sharpe 1.73) :
  Même signaux → même WR/PF approximatifs
  Levier plus élevé en compression → returns potentiellement plus grands
  Levier plus faible en volatilité → DD légèrement réduit
"""
...
# ---------------------------------------------------------------------------
# Constants — identiques v22 sauf levier
# ---------------------------------------------------------------------------
...
# Levier ATR percentile (nouveau vs v22's BASE=10/HIGH=15)
MAX_LEVERAGE = 20
MIN_LEVERAGE = 8
...
# Fenêtre fixe identique v22
BACKTEST_START = pd.Timestamp("2026-01-01")

CONFIGS = [
    {"name": "Prudent",  "risk_pct": 0.06, "max_pos": 2, "score_min": 68},
    {"name": "Optimal",  "risk_pct": 0.10, "max_pos": 3, "score_min": 64},
    {"name": "Agressif", "risk_pct": 0.14, "max_pos": 4, "score_min": 60},
]
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ATR percentile — levier inversement proportionnel
...
# ---------------------------------------------------------------------------
# Engine — v22 avec levier ATR percentile
# ---------------------------------------------------------------------------
...
def run_engine(sym_data_list, timestamps, risk_pct, max_pos, score_min):
    ...
    for ts in timestamps:
        ...
        for sd in sym_data_list:
            ...
            # Levier ATR percentile (vs v22's ADX/score threshold)
            atr_pctile = float(sd["atr_pctile"][bar])
            if not math.isnan(atr_pctile) and 0.0 <= atr_pctile <= 1.0:
                leverage = MAX_LEVERAGE - int((MAX_LEVERAGE - MIN_LEVERAGE) * atr_pctile)
            else:
                leverage = 13  # fallback ≈ moyenne v22
            if score >= 80:
                leverage = min(leverage + 2, MAX_LEVERAGE)

            # Sizing identique v22 (dd_scale + compound)
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v25 | ATR Leverage | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
    ...
    if monthly:
        ...
        for mk in sorted(monthly):
            ...
            mtbl.add_row(mk, str(len(tl)), f"{wr:.0%}", f"{ml_:.1f}×",
    ...
    if trades:
        etbl = Table(title="Sorties — TP risk vs SL risk (ARM check)", box=box.SIMPLE_HEAD)
        etbl.add_column("Raison",  style="cyan")
        etbl.add_column("Count",   justify="right")
        etbl.add_column("Win%",    justify="right")
        etbl.add_column("Lev moy", justify="right")
    ...
    console.print(
        f"\n  Mois profitables: "
        f"{sum(1 for v in monthly.values() if v >= 0)}/{len(monthly)}  |  "
...
def main():
    console.print(
        "\n[bold green]PRISM v25 — ATR Leverage | BB Squeeze | Fenêtre fixe Jan 2026[/bold green]"
    )
    console.print(
        "  Levier 8-20× ATR percentile | Risque & signals identiques v22\n"
        f"  Période fixe: {BACKTEST_START.date()} → 2026-06-20\n"
    )
    ...
    for cfg in CONFIGS:
        ...
        console.print(
            f"\n[bold]PRISM v25 — ATR Leverage | {name}[/bold]\n"
            f"  Risk: {risk_pct:.0%} (dd_scale v22) | "
            f"Levier 8-20× ATR | Score min: {score_min}"
        )
        ...
        # Intersection timestamps + filtre fenêtre fixe
        ts_sets = [set(sd["ts_index"]) for sd in sym_data_list]
        common  = ts_sets[0]
        for s in ts_sets[1:]:
            common &= s
        timestamps = sorted(ts for ts in common if ts >= BACKTEST_START)
        ...
        trades, eq_curve, _ = run_engine(
            sym_data_list, timestamps, risk_pct, max_pos, score_min
        )
        print_report(trades, eq_curve, name, t0)
        all_results.append((cfg, trades, eq_curve))

    # Tableau comparatif
    tbl = Table(
        title="Comparaison v25 — ATR Leverage | Fenêtre Jan 2026 | BB Squeeze",
        box=box.ROUNDED
    )
    ...
    best_sharpe, best_cfg_name, best_m = -999, "", {}
    ...
    console.print(tbl)
    rc = "green" if best_m.get("ret", 0) >= 0 else "red"
    console.print(
        f"\n  Meilleure config: [bold cyan]{best_cfg_name}[/bold cyan]  →  "
        f"€{INITIAL_CAPITAL:.0f} → €{best_m.get('final_equity',1000):.0f}  "
        f"(MaxDD: {best_m.get('max_dd',0):.1%})"
    )
    console.print("  Baseline v22 : +58.1%  MaxDD 23.2%  Sharpe 1.73")
    ret25 = best_m.get('ret', 0)
    if ret25 > 0.65:
        verdict = "✓ AMÉLIORATION vs v22 — levier ATR génère plus"
    elif ret25 > 0.50:
        verdict = "~ COMPARABLE à v22 — levier ATR neutre"
    else:
        verdict = "✗ Inférieur à v22 — investiguer"
    console.print(f"  v25 résultat  : {verdict}")
