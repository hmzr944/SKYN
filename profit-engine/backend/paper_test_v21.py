#!/usr/bin/env python3
"""
PRISM v21 — Mode Agressif | EMA Pullback + BB Squeeze 1H | €1000 → ×2
=======================================================================
Objectif : doubler le capital en 6 mois.
Levier 10-15×, Risk 10-15%/trade, Pattern A + C (1H), compound sizing.

Outil spécifique : ATR Dynamic Stop-Loss
  SL dynamique = ATR14 × 1.5 (s'adapte à la volatilité du marché)
  TP dynamique = SL × 4 (R:R 4:1 constant)
  → En marché calme : SL serré → risque réduit
  → En marché volatile : SL plus large → moins de stops prématurés

Paramètres vs v20 :
  Levier    : 4-6×  → 10-15×
  Risk/trade : 5.5% → 10-15%
  SL/TP     : fixe  → ATR dynamique (ATR×1.5 / ATR×6.0)
  Patterns  : A seulement → A + C (BB squeeze 1H, +30-40 trades)
  Score min : 65   → 62 (légèrement assoupli pour fréquence)
  EMA21 zone: 1.5% → 2.0% (légèrement assoupli)

Risque déclaré : MaxDD possible 25-40% — le revers du ×2.
"""
...
# ATR Dynamic Stop-Loss — l'outil spécifique
ATR_SL_MULT      = 1.5      # SL = 1.5 × ATR14
RR_RATIO         = 4.0      # TP = SL × RR_RATIO (R:R 4:1 dynamique)
ATR_SL_MIN_A     = 0.008    # plancher SL Pattern A (0.8% min)
ATR_SL_MAX_A     = 0.030    # plafond  SL Pattern A (3.0% max)
ATR_SL_MIN_C     = 0.006    # plancher SL Pattern C (0.6% min)
ATR_SL_MAX_C     = 0.025    # plafond  SL Pattern C (2.5% max)

# Pattern C — BB Squeeze 1H
SQUEEZE_BARS_C   = 5

# Commun
ADX_MIN_A        = 22
ADX_MIN_C        = 20
TIME_STOP_H      = 72        # 3 jours max
COOLDOWN_BARS    = 12        # 12h entre deux trades même symbole/pattern

EMA21_NEAR       = 0.020     # proche de l'EMA21 (assoupli vs v20 : 1.5%→2%)
EMA21_PRIOR      = 0.012     # était au moins à 1.2% de l'EMA21 avant

BASE_LEVERAGE    = 10        # 10× (vs 4× en v20)
HIGH_LEVERAGE    = 15        # 15× si ADX fort + score haut (vs 6× en v20)

DAILY_LOSS_CAP   = 0.12      # stoppe entrées si -12% dans la journée

CONFIGS = [
    {"name": "Modéré",     "risk_pct": 0.08,  "max_pos": 3, "score_min": 65},
    {"name": "Agressif",   "risk_pct": 0.12,  "max_pos": 4, "score_min": 62},
    {"name": "Ultra",      "risk_pct": 0.15,  "max_pos": 5, "score_min": 60},
]
...
# ---------------------------------------------------------------------------
# Pattern A — EMA Pullback 1H (v20, légèrement assoupli)
# ---------------------------------------------------------------------------
...
def _check_pattern_a(sd: dict, bar: int, adx_val: float) -> Optional[str]:
    ...
    try:
        ...
        if any(math.isnan(v) for v in [close, ema9, ema21, ema50, rsi, mh_cur,
                                        mh_prev, vol_r, ema20_4h, ema50_4h,
                                        ema50d, ema200d, adx_val]):
            return None

        prev5 = [
            abs(sd["close"][bar - i] - sd["ema21"][bar - i])
            / max(sd["ema21"][bar - i], 1e-10)
            for i in range(1, 6)
        ]
        max_dist    = max(prev5)
        ...
        if (ema9 > ema21 and ema21 > ema50
                ...
                and max_dist > EMA21_PRIOR
                and 42 <= rsi <= 63
                and mh_cur > mh_prev and mh_cur > 0
                and vol_r >= 0.9
                and adx_val >= ADX_MIN_A):
            return "BUY"
...
# ---------------------------------------------------------------------------
# Pattern C — BB Squeeze Breakout 1H (v16)
# ---------------------------------------------------------------------------
...
def _check_pattern_c(sd: dict, bar: int, adx_val: float) -> Optional[str]:
    ...
    try:
        ...
        # Squeeze sur SQUEEZE_BARS_C barres consécutives
        ...
        # Rupture de la compression
        if bbw_cur <= bbwq_cur:
            return None

        # ADX doit avoir accéléré
...
def run_engine(sym_data_list, timestamps, risk_pct, max_pos, score_min):
    ...
    for ts in timestamps:
        ...
        for pk in list(pending_entries.keys()):
            ...
            # ATR dynamic SL/TP — s'adapte à la volatilité courante
            atr_now = sd["atr14"][bar]
            if math.isnan(atr_now) or atr_now <= 0:
                atr_now = entry_price * 0.015  # fallback 1.5% si ATR manquant
            atr_pct = atr_now / (entry_price + 1e-10)
            if p["pattern"] == "A":
                sl_pct = max(ATR_SL_MIN_A, min(ATR_SL_MAX_A, ATR_SL_MULT * atr_pct))
            else:
        ...
        if skip_entries or len(open_positions) + len(pending_entries) >= max_pos:
            continue
        if drawdown_from_peak > 0.40:   # pause si DD > 40%
            continue
        ...
        for sd in sym_data_list:
            ...
            for pattern in ["A", "C"]:
                ...
                if pending_key not in pending_entries:
                    ...
                    break  # un seul pattern par symbole par barre
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v21 | Mode Agressif | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
    ...
    # Pattern breakdown
    if trades:
        ptbl = Table(title="Pattern A vs C", box=box.SIMPLE_HEAD)
        ptbl.add_column("Pattern", style="cyan")
        ptbl.add_column("Trades",  justify="right")
        ptbl.add_column("Win%",    justify="right")
        ptbl.add_column("Avg PnL", justify="right")
        ptbl.add_column("PF",      justify="right")
        for pat in ["A", "C"]:
            tl = [t for t in trades if t.pattern == pat]
            if not tl: continue
            ws  = sum(1 for t in tl if t.pnl_eur > 0)
            avg = sum(t.pnl_eur for t in tl) / len(tl)
            gw  = sum(t.pnl_eur for t in tl if t.pnl_eur > 0)
            gl  = abs(sum(t.pnl_eur for t in tl if t.pnl_eur <= 0))
            pf_ = gw / (gl + 1e-10)
            col = "green" if avg >= 0 else "red"
            name = "A — EMA Pullback" if pat == "A" else "C — BB Squeeze"
            ptbl.add_row(name, str(len(tl)), f"{ws/len(tl):.0%}",
                         f"[{col}]{avg:+.2f}[/]", f"{pf_:.2f}")
        console.print(ptbl)
...
def run_backtest(cfg_name: str = "Agressif") -> dict:
    ...
    console.print(f"\n[bold]PRISM v21 — Mode Agressif | {cfg_name}[/bold]")
    console.print(
        f"  OKX REST | {MONTHS} mois | {TIMEFRAME} | Capital €{INITIAL_CAPITAL:.0f}\n"
        f"  SL dynamique: ATR14 × {ATR_SL_MULT}  |  TP = SL × {RR_RATIO:.0f} (R:R 4:1)\n"
        f"  A: SL [{ATR_SL_MIN_A:.1%}-{ATR_SL_MAX_A:.1%}]  |  "
        f"C: SL [{ATR_SL_MIN_C:.1%}-{ATR_SL_MAX_C:.1%}]\n"
        f"  Risk/trade: {cfg['risk_pct']:.0%}  |  "
        f"Levier: {BASE_LEVERAGE}-{HIGH_LEVERAGE}×  |  "
        f"Score min: {cfg['score_min']}"
    )
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v21 — Mode Agressif | Levier 10-15× | Pattern A+C[/bold yellow]"
    )
    console.print(
        "[bold red]  ⚠  Rendement cible ×2 = risque MaxDD 25-40%  ⚠[/bold red]\n"
    )
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison configs — 6 mois | Levier 10-15×",
            box=box.ROUNDED
        )
