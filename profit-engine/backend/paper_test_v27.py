#!/usr/bin/env python3
"""
PRISM v27 — Itération 1 | Suppression symboles perdants
========================================================
Base : v27 (30 symboles, cooldown 5h, ADX/score leverage)
Delta v27 vs v27 :
  - FIL-USDT retiré (0% WR sur 2 trades, -€116 sur 6 mois)
  - ETH-USDT retiré (0% WR sur 3 trades, -€75 sur 6 mois)
  - ALGO-USDT retiré (0% WR sur 2 trades, -€51 sur 6 mois)
  - 27 symboles actifs (vs 30 en v27, vs 17 en v22)

Raisonnement : BB squeeze sur FIL/ETH/ALGO produit des faux breakouts
répétés dans ce régime de marché. Rediriger les slots vers symboles rentables.
"""
...
ADX_MIN_C        = 20
TIME_STOP_H      = 72
COOLDOWN_BARS    = 5       # v22: 8 → v27: 5 (plus de signaux)
...
def run_engine(sym_data_list, timestamps, risk_pct, max_pos, score_min):
    ...
    for ts in timestamps:
        ...
        for sd in sym_data_list:
            ...
            # Levier qualité (identique v22 — prouvé supérieur à ATR leverage)
            leverage = HIGH_LEVERAGE if (adx_val > 28 and score >= 72) else BASE_LEVERAGE
            # Compound sizing avec protection drawdown (identique v22)
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v27 | 27 Symboles | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
    ...
    if trades:
        ...
        stbl = Table(title="PnL par Symbole (v27 — 30 symboles)", box=box.SIMPLE_HEAD)
...
def run_backtest(cfg_name: str = "Agressif") -> dict:
    ...
    console.print(f"\n[bold]PRISM v27 — 27 Symboles | {cfg_name}[/bold]")
    console.print(
        ...
        f"  Symboles: {len(SYMBOLS)} (27 actifs, FIL/ETH/ALGO retirés)"
    )
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v27 — Scale-Out | 27 Symboles | Cooldown 5h[/bold yellow]"
    )
    console.print(
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison configs — 6 mois | 27 Symboles | Cooldown 5h",
            box=box.ROUNDED
        )
