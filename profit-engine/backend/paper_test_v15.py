#!/usr/bin/env python3
"""
SKYN v15 APEX — Stratégie Finale de Production
===============================================
Base : v14 (±2% macro, BTC-mom-4h, BTC-support-30j, 3F+5F)
       → +238.1%, DD 10.5%, WR 64%

Améliorations APEX :
  1. MTF 4H EMA20 — confirmation multi-timeframe : n'entre en SHORT
     que si le prix est sous la EMA20 des bougies 4H (tendance 4H baissière
     confirmée). Évite les short sur des rallies 4H dans un bear 1H.

  2. Support actif 30J par actif — comme le filtre BTC, chaque actif doit
     être à 3%+ au-dessus de son plancher de 30 jours pour shorter.
     Évite les entrées short sur des actifs déjà au fond.

  3. Partial TP en deux étapes :
       Étape 1 (40% du TP) → SL au point mort (breakeven)
       Étape 2 (70% du TP) → SL = entry − 30% du SL initial (profit garanti)
     Améliore l'EV sur les trades gagnants sans changer les entrées.

  4. ADX TP boost — si ADX ≥ 28 à l'entrée, TP × 1.15.
     Capture plus de profit dans les tendances particulièrement fortes.

  5. Nouvelles fonctions pour le dashboard :
       run_full_backtest(cfg_name) → dict structuré
       scan_live_signals(capital)  → signaux actuels

Cible : WR ≥ 70%, Retour ≥ +220%, DD ≤ 10%, Sharpe ≥ 2.5
"""
...
# ── v15 : Partial TP en deux étapes ───────────────────────────────────────────
PARTIAL_TP_FRAC1 = 0.40   # 40% du TP → SL au breakeven
PARTIAL_TP_FRAC2 = 0.70   # 70% du TP → SL = profit garanti
PARTIAL_TP_LOCK  = 0.30   # Fraction du SL initial verrouillée en profit

# ── v15 : ADX TP boost ────────────────────────────────────────────────────────
ADX_TP_BOOST_THR = 28.0   # ADX ≥ 28 → TP × 1.15
ADX_TP_BOOST_FAC = 1.15

# ── v15 : Multi-timeframe 4H ──────────────────────────────────────────────────
MTF_EMA_PERIOD = 20        # EMA 20 sur les bougies 4H

# ── v15 : Filtre support par actif ────────────────────────────────────────────
ASSET_SUPPORT_M = 0.03     # 3% au-dessus du plancher 30J pour shorter

# ── Filtres BTC macro (hérités v14) ───────────────────────────────────────────
BTC_MOM_BARS     = 4
BTC_MOM_THRESH   = 0.001
BTC_RSI_BEAR_MAX = 55
BTC_RSI_BULL_MIN = 45
BTC_DOWNSIDE_MIN = 0.05
BTC_UPSIDE_MIN   = 0.05

# ── Système de filtres ────────────────────────────────────────────────────────
ALLOWED_FILTER_COUNTS = {3, 5}
RISK_MULT_BY_F = {5: 2.5, 3: 1.0}
TP_PCT_BY_F    = {5: 0.070, 3: 0.045}

# ── 3 Configurations ──────────────────────────────────────────────────────────
CONFIGS = [
    {"name": "Precision", "score_min": 90, "sq_bars": 5, "risk_pct": 0.040, "sl_pct": 0.005},
    {"name": "Selectif",  "score_min": 85, "sq_bars": 5, "risk_pct": 0.045, "sl_pct": 0.005},
    {"name": "Optimal",   "score_min": 82, "sq_bars": 5, "risk_pct": 0.050, "sl_pct": 0.005},
]
...
def download_data(symbol: str, period: str = PERIOD) -> Optional[pd.DataFrame]:
    try:
...
def _daily_rsi(series: pd.Series, period: int = 14) -> pd.Series:
    delta    = series.diff()
...
def resample_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["ema50"]   = _ema(c, 50)
    d["ema200"]  = _ema(c, 200)
    d["rsi14d"]  = _daily_rsi(c, 14)
    d["low30d"]  = c.rolling(30, min_periods=10).min()
    d["high30d"] = c.rolling(30, min_periods=10).max()
    return d
...
def precompute_symbol(sym: str, cfg) -> Optional[dict]:
    ...
    # ── v15 : MTF 4H EMA20 ────────────────────────────────────────────────────
    _c4h         = df["close"].resample("4h").last().dropna()
    _ema20_4h    = _c4h.ewm(span=MTF_EMA_PERIOD, adjust=False).mean()
    df["ema20_4h"] = _ema20_4h.reindex(df.index, method="ffill").fillna(df["close"])

    # ── v15 : Support/résistance 30J par actif (720 barres 1h) ───────────────
    df["low30h"]  = df["close"].rolling(720, min_periods=120).min().fillna(df["close"] * 0.5)
    df["high30h"] = df["close"].rolling(720, min_periods=120).max().fillna(df["close"] * 2.0)
...
def _build_btc_macro(btc_sd: dict) -> Tuple[Dict, Dict, Dict, Dict]:
    """
    v14/v15 : retourne (macro_state, daily_rsi, btc_low30, btc_high30).
    ...
    for ts, row in df_d.iterrows():
        try:
            ...
            rsi   = float(row.get("rsi14d", 50) or 50)
            low30 = float(row.get("low30d",  0) or 0)
            hi30  = float(row.get("high30d", 0) or 0)
            if e50 <= 0 or e200 <= 0:
                state = "neutral"
            else:
                ratio = e50 / e200
                if   ratio > 1.02: state = "bull"
                elif ratio < 0.98: state = "bear"
                else:              state = "neutral"
        except Exception:
            state = "neutral"; rsi = 50; low30 = 0; hi30 = 0

        result[ts.date()]    = state
        rsi_map[ts.date()]   = rsi
        low30_map[ts.date()] = low30
        high30_map[ts.date()] = hi30

    return result, rsi_map, low30_map, high30_map
...
def _check_breakout(sd: dict, bar: int, adx_val: float, adx_series: pd.Series,
    ...
    try:
        ...
        ema20_4h = float(last.get("ema20_4h", price) or price)
        low30h   = float(last.get("low30h",  price * 0.5) or price * 0.5)
        high30h  = float(last.get("high30h", price * 2.0) or price * 2.0)
    ...
    # ── v15 : MTF 4H EMA alignment ────────────────────────────────────────────
    if ema20_4h > 0 and price > 0:
        if action == "SELL" and price > ema20_4h * 0.997:
            return 0, None   # 4h trend encore haussier — skip short
        if action == "BUY"  and price < ema20_4h * 1.003:
            return 0, None   # 4h trend encore baissier — skip long

    # ── v15 : Support/résistance 30J par actif ────────────────────────────────
    if action == "SELL" and low30h > 0:
        if price < low30h * (1.0 + ASSET_SUPPORT_M):
            return 0, None   # actif trop proche de son plancher 30J
    if action == "BUY" and high30h > 0:
        if price > high30h * (1.0 - ASSET_SUPPORT_M):
            return 0, None   # actif trop proche de sa résistance 30J
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
               btc_macro: Dict, btc_rsi: Dict, btc_low30: Dict, btc_high30: Dict,
    ...
    for ts in common_timestamps:
        ...
        btc_rsi_day  = btc_rsi.get(pd.Timestamp(day_key).date(), 50.0)
        btc_low30_d  = btc_low30.get(pd.Timestamp(day_key).date(), 0.0)
        btc_hi30_d   = btc_high30.get(pd.Timestamp(day_key).date(), 0.0)
        ...
        for sym in list(positions.keys()):
            ...
            # Liquidation
            if side == "long"  and low  <= pos["liq"]: reason = "liquidation"
            elif side == "short" and high >= pos["liq"]: reason = "liquidation"

            # Stop loss (peut avoir bougé via partial TP)
            if reason is None:
                if side == "long"  and low  <= pos["sl"]: reason = "stop_loss"
                elif side == "short" and high >= pos["sl"]: reason = "stop_loss"

            # ── v15 : Partial TP étape 1 (40%) → breakeven ────────────────────
            if reason is None and not pos.get("partial1_taken"):
                if side == "long"  and high >= pos["partial_tp1"]:
                    pos["partial1_taken"] = True
                    pos["sl"] = pos["entry"]
                elif side == "short" and low  <= pos["partial_tp1"]:
                    pos["partial1_taken"] = True
                    pos["sl"] = pos["entry"]

            # ── v15 : Partial TP étape 2 (70%) → verrouiller profit ───────────
            if reason is None and pos.get("partial1_taken") and not pos.get("partial2_taken"):
                sl_lock = PARTIAL_TP_LOCK * abs(pos["sl_orig"] - pos["entry"])
                if side == "long"  and high >= pos["partial_tp2"]:
                    pos["partial2_taken"] = True
                    pos["sl"] = pos["entry"] + sl_lock   # profit garanti long
                elif side == "short" and low  <= pos["partial_tp2"]:
                    pos["partial2_taken"] = True
                    pos["sl"] = pos["entry"] - sl_lock   # profit garanti short

            # Take profit complet
            if reason is None:
                if side == "long"  and high >= pos["tp"]: reason = "take_profit"
                elif side == "short" and low  <= pos["tp"]: reason = "take_profit"
            ...
            if reason == "liquidation":
                ...
                if reason == "stop_loss":
                    exit_p = tgt * (1 + SLIPPAGE) if side == "long" \
                             else tgt * (1 - SLIPPAGE)
                else:
                    exit_p = tgt * (1 - SLIPPAGE) if side == "long" \
                             else tgt * (1 + SLIPPAGE)
            ...
            all_trades.append(PaperTrade(
                ...
                adx_entry=pos.get("adx_entry", 0.0),
            ))
            if pnl > 0:
        ...
        # v14 : RSI daily BTC
        if macro == "bear" and btc_rsi_day > BTC_RSI_BEAR_MAX:
            continue
        if macro == "bull" and btc_rsi_day < BTC_RSI_BULL_MIN:
            continue

        # v14 : BTC support 30J (ne short pas au fond)
        btc_sd  = sym_data.get("BTC-USD")
        btc_bar = btc_sd["ts_to_pos"].get(ts) if btc_sd else None
        if btc_sd is not None and btc_bar is not None and btc_low30_d > 0:
            btc_close = float(btc_sd["df"].iloc[btc_bar]["close"])
            if macro == "bear" and btc_close < btc_low30_d * (1 + BTC_DOWNSIDE_MIN):
                continue
            if macro == "bull" and btc_hi30_d > 0 and btc_close > btc_hi30_d * (1 - BTC_UPSIDE_MIN):
                continue

        # v14 : BTC 4h momentum
        if btc_sd is not None and btc_bar is not None and btc_bar >= BTC_MOM_BARS:
            btc_now  = float(btc_sd["df"].iloc[btc_bar]["close"])
            btc_prev = float(btc_sd["df"].iloc[btc_bar - BTC_MOM_BARS]["close"])
            if btc_prev > 0:
                btc_chg = (btc_now - btc_prev) / btc_prev
                if macro == "bear" and btc_chg > BTC_MOM_THRESH:
                    continue
                if macro == "bull" and btc_chg < -BTC_MOM_THRESH:
                    continue
        ...
        for sym, sd in sym_data.items():
            ...
            candidates.append({"score": dir_sc, "filters": nf, "sym": sym,
                                "action": action, "bar": bar, "adx": adx_i})
        ...
        for cand in candidates[:slots]:
            ...
            # ── v15 : ADX TP boost ────────────────────────────────────────────
            tp_mult    = ADX_TP_BOOST_FAC if cand["adx"] >= ADX_TP_BOOST_THR else 1.0
            tp_dist_eff = tp_dist * tp_mult
            ...
            if cand["action"] == "BUY":
                ...
                tp_p        = entry_p * (1 + tp_dist_eff)
                partial_tp1 = entry_p * (1 + tp_dist_eff * PARTIAL_TP_FRAC1)
                partial_tp2 = entry_p * (1 + tp_dist_eff * PARTIAL_TP_FRAC2)
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    eq_vals  = [v for _, v in w_eq]
    final_eq = eq_vals[-1]; start_v = eq_vals[0]
    ...
    console.print()
    console.rule("[bold yellow]SKYN v15 APEX — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    params_str = (f"score≥{params['score_min']} | sq≥{params['sq_bars']}h | "
                  f"risk={params['risk_pct']*100:.1f}% | sl={params['sl_pct']*100:.1f}%")
    console.print(Panel(
    ...
    df_t  = pd.DataFrame([{"month": str(t.entry_ts)[:7], "pnl": t.pnl_eur,
    ...
    if w_trades:
        ...
        for c in ["Sym", "Side", "Score", "F", "ADX", "Net EUR", "PnL%", "Lev", "Sortie"]:
            tw.add_column(c)
        for t in top:
            tw.add_row(
                t.symbol.split("/")[0], t.side[:5],
                str(t.score), f"{t.filters}/5", f"{t.adx_entry:.0f}",
    ...
    console.rule(
        f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades · "
        f"macro±2% · MTF-4H · asset-support · 2-stage-TP · ADX-boost[/bold]"
    )


# ── Dashboard API : Backtest complet ─────────────────────────────────────────

def run_full_backtest(cfg_name: str = "Selectif") -> dict:
    """
    Lance le backtest complet 6 mois et retourne un dict structuré
    pour le dashboard. Prend ~90s (téléchargement inclus).
    """
    t0  = time.time()
    cfg = AppConfig()
    cfg_params = next((c for c in CONFIGS if c["name"] == cfg_name), CONFIGS[1])
    ...
    if "BTC-USD" not in sym_data:
        return {"error": "BTC-USD unavailable"}

    btc_macro, btc_rsi, btc_low30, btc_high30 = _build_btc_macro(sym_data["BTC-USD"])
    ...
    trades, eq_curve, day_wins = run_engine(
        sym_data, window_timestamps, btc_macro, btc_rsi, btc_low30, btc_high30,
        score_min=cfg_params["score_min"], sq_bars=cfg_params["sq_bars"],
        risk_pct=cfg_params["risk_pct"], sl_pct=cfg_params["sl_pct"],
    )

    w_trades = [t for t in trades if t.entry_ts >= window_start]
    w_eq     = [(ts, v) for ts, v in eq_curve if ts >= window_start]

    if not w_eq:
        return {"error": "No trades in 6-month window"}
    ...
    wr     = len(wins) / len(w_trades) * 100 if w_trades else 0
    gw     = sum(t.pnl_eur for t in wins)  if wins   else 0
    gl     = abs(sum(t.pnl_eur for t in losses)) if losses else 1e-9
    pf     = gw / gl
    ...
    # Downsample equity curve to 400 points for dashboard
    step   = max(1, len(w_eq) // 400)
    eq_ds  = w_eq[::step]
    eq_out = [{"ts": str(ts), "equity": round(v, 2)} for ts, v in eq_ds]

    # Monthly breakdown
    df_t  = pd.DataFrame([{"month": str(t.entry_ts)[:7], "pnl": t.pnl_eur,
                            "win": t.pnl_eur > 0} for t in w_trades])
    eq_df = pd.DataFrame(w_eq, columns=["ts", "eq"])
    eq_df["month"] = eq_df["ts"].dt.strftime("%Y-%m")
    monthly = []
    prev_eq = start_v
    for m in sorted(df_t["month"].unique() if len(df_t) > 0 else []):
        mt    = df_t[df_t["month"] == m]
        m_eq  = eq_df[eq_df["month"] == m]["eq"]
        end_e = float(m_eq.iloc[-1]) if len(m_eq) > 0 else prev_eq
        ret_m = (end_e - prev_eq) / prev_eq * 100 if prev_eq > 0 else 0
        monthly.append({
            "month":   m,
            "trades":  len(mt),
            "win_pct": round(mt["win"].sum() / max(len(mt), 1) * 100, 1),
            "pnl_eur": round(float(mt["pnl"].sum()), 2),
            "ret_pct": round(ret_m, 1),
            "capital": round(end_e, 2),
        })
        prev_eq = end_e
    ...
    trades_out = [{
        ...
        "margin_eur":  round(t.margin_eur,  2),
        "pnl_eur":     round(t.pnl_eur,     2),
        "pnl_pct":     round(t.pnl_pct,     2),
        "exit_reason": t.exit_reason,
        "score":       t.score,
        "filters":     t.filters,
        "leverage":    t.leverage,
        "adx_entry":   round(t.adx_entry,   1),
    } for t in sorted(w_trades, key=lambda x: x.entry_ts)]
    ...
    return {
        "config":       cfg_params,
        "window":       {"start": str(window_start)[:10], "end": str(last_ts)[:10]},
        "elapsed_s":    round(elapsed, 1),
        "macro_counts": macro_counts,
        "metrics": {
            "return_pct":    round(ret_pct,  2),
            "start_equity":  round(start_v,  2),
            "end_equity":    round(end_v,    2),
            ...
            "avg_leverage":  round(sum(t.leverage for t in w_trades) / max(len(w_trades), 1), 1),
            "liquidations":  sum(1 for t in w_trades if t.exit_reason == "liquidation"),
            "f5_count":      sum(1 for t in w_trades if t.filters == 5),
            "f3_count":      sum(1 for t in w_trades if t.filters == 3),
        },
        "equity_curve":  eq_out,
        "monthly":       monthly,
        "trades":        trades_out,
...
# ── Dashboard API : Scanner live ─────────────────────────────────────────────

def scan_live_signals(capital: float = 500.0, cfg_name: str = "Selectif") -> dict:
    """
    Télécharge les données récentes et scanne les 17 symboles pour
    les signaux actuels. Retourne un dict structuré pour le dashboard.
    Prend ~60-90s.
    """
    t0          = time.time()
    cfg         = AppConfig()
    cfg_params  = next((c for c in CONFIGS if c["name"] == cfg_name), CONFIGS[1])
    score_min   = cfg_params["score_min"]
    sq_bars     = cfg_params["sq_bars"]
    sl_pct      = cfg_params["sl_pct"]
    ...
    # Timestamp actuel (dernier bar commun)
    common_set = None
    for sd in sym_data.values():
        s = set(sd["ts_index"])
        common_set = s if common_set is None else common_set & s
    common_ts = sorted(common_set)
    if not common_ts:
    ...
    # BTC info actuel
    btc_sd   = sym_data["BTC-USD"]
    btc_bar  = btc_sd["ts_to_pos"].get(ts)
    btc_price = float(btc_sd["df"].iloc[btc_bar]["close"]) if btc_bar else 0.0
    macro     = btc_macro.get(pd.Timestamp(day_k).date(), "neutral")
    rsi_d     = btc_rsi.get(pd.Timestamp(day_k).date(), 50.0)
    low30_d   = btc_low30.get(pd.Timestamp(day_k).date(), 0.0)

    # Vérifications BTC macro globales
    btc_blocked_reason = None
    if macro == "neutral":
        btc_blocked_reason = "Macro neutre — pas d'entrées"
    elif macro == "bear" and rsi_d > BTC_RSI_BEAR_MAX:
        btc_blocked_reason = f"RSI daily BTC {rsi_d:.0f} > {BTC_RSI_BEAR_MAX} — recovery en cours"
    elif btc_bar is not None and btc_bar >= BTC_MOM_BARS:
        btc_now  = btc_price
        btc_prev = float(btc_sd["df"].iloc[btc_bar - BTC_MOM_BARS]["close"])
        btc_chg  = (btc_now - btc_prev) / btc_prev if btc_prev > 0 else 0
        if macro == "bear" and btc_chg > BTC_MOM_THRESH:
            btc_blocked_reason = f"BTC 4h momentum haussier (+{btc_chg*100:.2f}%)"
    if btc_bar is not None and low30_d > 0:
        if macro == "bear" and btc_price < low30_d * (1 + BTC_DOWNSIDE_MIN):
            btc_blocked_reason = f"BTC trop proche du support 30J ({btc_price:.0f} < {low30_d*(1+BTC_DOWNSIDE_MIN):.0f})"
    ...
    for sym, sd in sym_data.items():
        ...
        row     = sd["df"].iloc[bar]
        price   = float(row["close"])
        ema4h   = float(row.get("ema20_4h", price) or price)
        low30h  = float(row.get("low30h",   price * 0.5) or price)
        cur_bbw = float(row.get("bbw", 1.0) or 1.0)
        vol_r   = float(row.get("vol_ratio", 1.0) or 1.0)
        b_score = int(sd["buy_sc"][bar])
        s_score = int(sd["sell_sc"][bar])

        sym_name = sd["name"]

        if nf == 0 or action is None:
            rejected.append({
                "symbol":  sym_name,
                "price":   round(price, 4),
                "b_score": b_score,
        ...
        if btc_blocked_reason:
            rejected.append({
                "symbol":  sym_name,
                "action":  action,
                "filters": nf,
                "price":   round(price, 4),
                "reason":  btc_blocked_reason,
            })
            continue

        if macro == "bull" and action == "SELL":
            rejected.append({"symbol": sym_name, "action": action, "filters": nf,
                              "price": round(price, 4), "reason": "Direction vs macro bull"})
            continue
        if macro == "bear" and action == "BUY":
            rejected.append({"symbol": sym_name, "action": action, "filters": nf,
                              "price": round(price, 4), "reason": "Direction vs macro bear"})
            continue

        dir_score = b_score if action == "BUY" else s_score
        tp_pct    = TP_PCT_BY_F.get(nf, 0.045)
        tp_mult   = ADX_TP_BOOST_FAC if adx_i >= ADX_TP_BOOST_THR else 1.0
        risk_mult = RISK_MULT_BY_F.get(nf, 1.0)
        risk_eur  = capital * cfg_params["risk_pct"] * risk_mult
        lev       = _get_lev(dir_score)

        if action == "BUY":
            entry = price * (1 + SLIPPAGE)
            sl    = entry * (1 - sl_pct)
            tp    = entry * (1 + tp_pct * tp_mult)
        else:
            entry = price * (1 - SLIPPAGE)
            sl    = entry * (1 + sl_pct)
            tp    = entry * (1 - tp_pct * tp_mult)

        pct_above_support = (price / low30h - 1) * 100 if low30h > 0 else 0
        pct_from_ema4h    = (price / ema4h - 1) * 100 if ema4h > 0 else 0
        ...
        signals.append({
            ...
            "leverage":      lev,
            "vol_surge":     round(vol_r, 1),
            "bbw":           round(cur_bbw, 4),
            "pct_above_sup": round(pct_above_support, 1),
            "pct_from_ema4h": round(pct_from_ema4h, 2),
        })
    ...
    return {
        ...
        "btc_rsi":        round(rsi_d, 1),
        "btc_blocked":    btc_blocked_reason,
        "screened":       len(sym_data) - 1,
        "signals":        sorted(signals, key=lambda s: (s["filters"], s["score"]), reverse=True),
...
def run_configs(sym_data, window_timestamps, btc_macro, btc_rsi, btc_low30, btc_high30, window_start):
    results = []
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"(3F+5F, macro±2%, MTF-4H, asset-support, {N_MAX_POSITIONS} pos)...[/bold cyan]")

    for cfg in CONFIGS:
        t_s = time.time()
        console.print(f"  [{cfg['name']}] score≥{cfg['score_min']} "
                      f"sq≥{cfg['sq_bars']} risk={cfg['risk_pct']*100:.0f}% "
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v15 APEX — MTF·4H·ASSET-SUP·2-STAGE-TP·ADX-BOOST[/bold yellow]")
    ...
    for sym in SYMBOLS_YF:
        ...
        sd = precompute_symbol(sym, cfg)
        if sd is None:
            console.print(f"  [yellow]? {sym} — indisponible[/yellow]")
            continue
        sym_data[sym] = sd
        console.print(f"  [green]✓ {sym}[/green] — {len(sd['df'])} barres [{time.time()-t_sym:.1f}s]")

    if "BTC-USD" not in sym_data:
        console.print("[bold red]BTC-USD obligatoire.[/bold red]")
        return
    ...
    n = macro_counts.get("neutral", 0); total_d = b + r + n or 1
    console.print(f"\n  Macro BTC (2 ans, ±2%) : bull={b}j  bear={r}j  neutral={n}j  "
    ...
    results = run_configs(sym_data, window_timestamps, btc_macro, btc_rsi, btc_low30, btc_high30, window_start)

    console.print()
    cmp_table = Table(title="Comparatif v15 APEX — 3 Configurations", box=box.SIMPLE_HEAVY)
    for col in ["Config", "Score≥", "Sq≥", "Risk%", "Ret%", "DD%", "WR%", "Trades", "Net EUR"]:
    ...
    console.print(f"\n[bold green]▶ Meilleure config : {best['name']} "
                  f"(score≥{best['score_min']} sq≥{best['sq_bars']} risk={best['risk_pct']*100:.0f}%)[/bold green]")

    print_report(best["trades_obj"], best["eq_curve"], best["day_wins"],
