#!/usr/bin/env python3
"""
PRISM v23 — Adaptive Risk Engine | BB Squeeze | ARM Minimaliste
===============================================================
Philosophie : partir du niveau v22 (base_risk = 14%) et seulement
dévier quand il y a une raison forte.

ARM — 2 mécanismes uniquement :
  1. Drawdown protection    — coupe 20-70% si en DD (non-négociable)
  2. Streak anti-martingale — boost ×1.15-1.50 sur wins, coupe ×0.40-0.65 sur losses
  En conditions neutres → risk = BASE_RISK (identique v22)

Levier dynamique 8-20× (ATR percentile, comme v22-v23).

Vs v22 :
  + ARM coupe le risk en mai 2026 (3-4 losses consécutives → risk ×0.40)
  + ARM booste en janv/fév sur winning streaks (risk ×1.15-1.30)
  + 3 nouveaux symboles (ARB, APT, SUI) = plus d'opportunités
  = Attendu : return ≥ v22 (+58%) avec MaxDD ≤ v22 (23%)
"""
...
# ---------------------------------------------------------------------------
# Constants — identiques v22 sauf +3 symboles
# ---------------------------------------------------------------------------
...
# ATR dynamic SL/TP (v22 identique)
ATR_SL_MULT      = 1.5
RR_RATIO         = 4.0
ATR_SL_MIN       = 0.006
ATR_SL_MAX       = 0.025
...
# BB Squeeze — paramètres IDENTIQUES v22
SQUEEZE_BARS     = 4
VOL_RATIO_MIN    = 1.5
ADX_ACCEL_MIN    = 1.5
ADX_MIN_C        = 20
...
# Configs — base_risk = niveau v22 élevé, ARM ajuste autour
CONFIGS = [
    {"name": "Conservateur", "base_risk": 0.08, "max_risk": 0.14, "max_pos": 2, "score_min": 68},
    {"name": "Optimal",      "base_risk": 0.12, "max_risk": 0.20, "max_pos": 3, "score_min": 64},
    {"name": "Agressif",     "base_risk": 0.14, "max_risk": 0.24, "max_pos": 4, "score_min": 60},
]
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ATR percentile (0=calme, 1=volatile) sur fenêtre 100 barres
    atr_rel = atr14 / (close + 1e-10)
    df["atr_pctile"] = atr_rel.rolling(100).apply(
...
def precompute(sym: str) -> Optional[dict]:
    ...
    sd = {
        ...
        "atr_pctile": df["atr_pctile"].values.astype(float),
...
# ---------------------------------------------------------------------------
# Pattern C — BB Squeeze (paramètres identiques v22)
# ---------------------------------------------------------------------------
...
def _check_pattern_c(sd: dict, bar: int, adx_val: float) -> Optional[str]:
    ...
    try:
        ...
        if math.isnan(adx_val) or math.isnan(adx_prev):
            return None
        if adx_val <= adx_prev + ADX_ACCEL_MIN or adx_val < ADX_MIN_C:
            return None
        close    = sd["close"][bar]
...
# ---------------------------------------------------------------------------
# Adaptive Risk Manager — Minimaliste (drawdown + streak uniquement)
# ---------------------------------------------------------------------------
...
class AdaptiveRiskManager:
    """
    ARM minimaliste : en conditions neutres = base_risk (= v22).
    Seuls deux mécanismes dérogent :
      • drawdown protection (coupe si en DD)
      • streak anti-martingale (boost/coupe sur séries wins/losses)
    """

    def __init__(self, base_risk: float, max_risk: float):
        self.base_risk = base_risk
        self.max_risk  = max_risk

    @staticmethod
    def _count_consecutive_from_end(trades: List[Trade], want_wins: bool) -> int:
        count = 0
        for t in reversed(trades[-6:]):
            if (t.pnl_eur > 0) == want_wins:
                count += 1
            else:
                break
        return count
    ...
    def compute(
        ...
        """Retourne (risk_pct, leverage)."""
        dd = (equity_peak - equity) / (equity_peak + 1e-10)

        # ── Drawdown (protection absolue) ────────────────────────────────
        if dd >= 0.28: return 0.0, 0
        if dd >= 0.18: risk_mult = 0.22
        elif dd >= 0.12: risk_mult = 0.48
        elif dd >= 0.06: risk_mult = 0.75
        else:            risk_mult = 1.00

        # ── Streak anti-martingale (seulement si pas en DD) ───────────────
        if dd < 0.06 and len(recent_trades) >= 2:
            cw = self._count_consecutive_from_end(recent_trades, want_wins=True)
            cl = self._count_consecutive_from_end(recent_trades, want_wins=False)

            # Boost sur winning streak
            if cw >= 4:   risk_mult *= 1.55
            elif cw >= 3: risk_mult *= 1.32
            elif cw >= 2: risk_mult *= 1.15

            # Coupe sur losing streak
            if cl >= 4:   risk_mult *= 0.35
            elif cl >= 3: risk_mult *= 0.48
            elif cl >= 2: risk_mult *= 0.68

        risk = max(MIN_RISK, min(self.max_risk, self.base_risk * risk_mult))

        # ── Levier dynamique inversement proportionnel à ATR ─────────────
        if not math.isnan(atr_pctile) and 0 <= atr_pctile <= 1:
            lev = MAX_LEVERAGE - int((MAX_LEVERAGE - MIN_LEVERAGE) * atr_pctile)
        else:
            lev = 12
        if score >= 80:
            lev = min(lev + 2, MAX_LEVERAGE)
...
def run_engine(sym_data_list, timestamps, base_risk, max_risk, max_pos, score_min):
    equity           = INITIAL_CAPITAL
    open_positions   = {}
    pending_entries  = {}
    trades: List[Trade] = []
    eq_curve         = [float(equity)]
    day_wins         = {}
    current_day      = ""
    day_start_equity = INITIAL_CAPITAL
    equity_peak      = INITIAL_CAPITAL
    cooldown_tracker = {}
    arm              = AdaptiveRiskManager(base_risk=base_risk, max_risk=max_risk)
    ...
    for ts in timestamps:
        ...
        for pk in list(pending_entries.keys()):
            ...
            sl_pct  = max(ATR_SL_MIN, min(ATR_SL_MAX, ATR_SL_MULT * atr_pct))
        ...
        if drawdown_from_peak >= 0.28:
            continue

        # BTC macro filter (identique v22)
        btc_4h_bull = None
        ...
        for sd in sym_data_list:
            ...
            ck       = sym + "C"
            last_bar = cooldown_tracker.get(ck, -9999)
            ...
            atr_pctile = float(sd["atr_pctile"][bar])

            risk_pct, leverage = arm.compute(
                equity        = equity,
            ...
            pending_key = sym + "C"
            if pending_key not in pending_entries:
                pending_entries[pending_key] = {
                    "sym":      sym,
...
def _metrics(trades: List[Trade], eq_curve: list) -> dict:
    if not eq_curve:
        return {"n_trades": 0, "ret": 0.0, "max_dd": 0.0,
                "win_rate": 0.0, "net_pnl": 0.0, "final_equity": INITIAL_CAPITAL,
                "profit_factor": 0.0, "sharpe": 0.0, "avg_risk": 0.0, "avg_lev": 0.0}
    ...
    return {
        ...
        "sharpe":        sharpe,
        "monthly":       monthly,
        "avg_risk":      sum(t.risk_pct for t in trades) / n,
        "avg_lev":       sum(t.leverage for t in trades) / n,
    }
...
def print_report(trades: List[Trade], eq_curve: list, cfg: dict, t0: float):
    m  = _metrics(trades, eq_curve)
    rc = "green" if m["ret"] >= 0 else "red"

    console.print(
        f"\n[bold cyan]━━━ PRISM v23 | ARM Adaptatif | {cfg['name']} ━━━[/bold cyan]"
    )
    console.print(
        f"  €{INITIAL_CAPITAL:.0f} → [bold]€{m['final_equity']:.2f}[/bold]  "
        f"Retour: [{rc}]{m['ret']:+.1%}[/]  "
    ...
    if m["n_trades"] > 0:
        console.print(
            f"  Risk moyen adaptatif: {m['avg_risk']:.1%}  |  "
            f"Levier moyen: {m['avg_lev']:.1f}×  |  "
            f"Base risk: {cfg['base_risk']:.0%}"
        )
    ...
    if monthly:
        ...
        mtbl.add_column("Risk moy",justify="right")
        mtbl.add_column("Lev moy", justify="right")
        ...
        for mk in sorted(monthly):
            ...
            ar  = sum(t.risk_pct for t in tl) / len(tl) if tl else 0
            al  = sum(t.leverage for t in tl) / len(tl) if tl else 0
            pnl = monthly[mk]
            ret = pnl / (eq_running + 1e-10)
            eq_running += pnl
            col = "green" if pnl >= 0 else "red"
            mtbl.add_row(mk, str(len(tl)), f"{wr:.0%}", f"{ar:.1%}", f"{al:.1f}×",
    ...
    if trades:
        ...
        for reason in ["take_profit", "stop_loss", "time_stop"]:
            ...
            ar  = sum(t.risk_pct for t in tl) / len(tl)
...
def run_backtest(cfg: dict) -> dict:
    t0 = time.time()
    console.print(f"\n[bold]PRISM v23 — ARM Adaptatif | {cfg['name']}[/bold]")
    console.print(
        f"  Base risk: {cfg['base_risk']:.0%} → adaptatif [{MIN_RISK:.0%}–{cfg['max_risk']:.0%}] "
        f"| Levier {MIN_LEVERAGE}-{MAX_LEVERAGE}× | Score min: {cfg['score_min']}"
    )
    ...
    print_report(trades, eq_curve, cfg, t0)
    return {"config": cfg["name"], "metrics": metrics}
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v23 — ARM Adaptatif | BB Squeeze | 20 symboles[/bold yellow]"
    )
    console.print(
        "[bold cyan]  ARM : base_risk en conditions neutres | boost wins | coupe losses | DD protection[/bold cyan]\n"
    )

    results = []
    for cfg in CONFIGS:
        r = run_backtest(cfg=cfg)
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison v23 — 6 mois | ARM Adaptatif | BB Squeeze 20 symboles",
            box=box.ROUNDED
        )
        ...
        tbl.add_column("Risk moy", justify="right")
        tbl.add_column("Lev moy",  justify="right")
        for name, m in results:
            rc     = "green" if m["ret"] >= 0 else "red"
            dd_col = "red" if m["max_dd"] > 0.25 else "yellow" if m["max_dd"] > 0.12 else "green"
            tbl.add_row(
                name,
                str(m["n_trades"]),
                f"[{rc}]{m['ret']:+.1%}[/]",
                ...
                f"{m.get('sharpe',0):.2f}",
                f"{m.get('avg_risk',0):.1%}",
                f"{m.get('avg_lev',0):.1f}×",
            )
        console.print(tbl)
        ...
        if bm["ret"] >= 1.0:
            ...
            console.print(f"\n[yellow]  ~ +{bm['ret']:.0%} — très bon (>×1.5)[/yellow]")
        elif bm["ret"] > 0:
            console.print(f"\n[yellow]  ~ Profitable (+{bm['ret']:.0%})[/yellow]")
        else:
            console.print(f"\n[red]  ✗ Perte — marché défavorable sur cette période[/red]")
