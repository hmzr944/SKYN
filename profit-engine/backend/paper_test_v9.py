#!/usr/bin/env python3
"""
SKYN v9 — CHIRURGIE SUR LES FAIBLESSES v8
==========================================
Objectif : éliminer les pertes identifiées, cibler +200%/mois.

Leçons v8 :
  ✗ 4F trades = WR 14% (pire que aléatoire!) → SKIP nf==4
  ✗ Macro neutral = 0% WR sur Apr-Jun       → SKIP neutral

Nouveautés vs v8 :
  - Skip 4F (nf==4) : seulement 3F et 5F acceptés
  - Macro neutral : aucune entrée (trop risqué sans direction claire)
  - TP plus élevé : 3F=4.5%, 5F=6.5% (RR 9:1 et 13:1)
  - Risk 5F = 2.5× base (conviction maximale quand 5 filtres)
  - Config "Precision" ajoutée (score≥90, sq≥5)
  - 17 symboles (APT toujours indisponible)
"""
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v9 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v9[/bold cyan]",
        border_style="cyan",
    ))
    ...
    console.rule(f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades · 4F exclus · neutral skip[/bold]")
...
def run_configs(sym_data, window_timestamps, btc_macro, window_start):
    ...
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"(3F+5F only, no neutral, {N_MAX_POSITIONS} positions)...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v9 — CHIRURGIE PRECISION : 3F+5F / NO NEUTRAL[/bold yellow]")
    ...
    macro_counts = {"bull": 0, "bear": 0, "neutral": 0}
    for v in btc_macro.values():
        macro_counts[v] = macro_counts.get(v, 0) + 1
    console.print(f"\n  Macro BTC (2 ans) : "
                  f"bull={macro_counts['bull']}j  "
                  f"bear={macro_counts['bear']}j  "
                  f"neutral={macro_counts['neutral']}j")
    ...
    console.print()
    cmp_table = Table(title="Comparatif v9 — 3 Configurations (4F exclu, neutral skip)",
