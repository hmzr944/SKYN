#!/usr/bin/env python3
"""
PRISM v22 — BB Squeeze Focus | Pattern C Only | ATR Dynamic SL
===============================================================
Stratégie : BB Squeeze Breakout 1H uniquement (le meilleur pattern v21).

Pourquoi Pattern C seulement ?
  - v21 Pattern C : WR 38-42%, PF 1.47-1.80  ✓ rentable
  - v21 Pattern A : WR 23-25%, PF ~1.0        ✗ dilue les résultats

Optimisations v22 vs v21 :
  - Pattern A entièrement retiré
  - Volume ratio assoupli : 2.0 → 1.5 (plus de signaux)
  - SQUEEZE_BARS : 5 → 4 (squeeze plus court = plus de breakouts)
  - Cooldown réduit : 12 → 8 barres (récupère les séries)
  - ATR SL conservé : SL = ATR14 × 1.5, TP = SL × 4

Outil spécifique : ATR Dynamic Stop-Loss
  SL s'adapte à la volatilité → marché calme = SL serré = risque réduit.
"""
...
SQUEEZE_BARS_C   = 4         # squeeze 4 barres (v21: 5) → plus de signaux
VOL_RATIO_C      = 1.5       # volume ratio (v21: 2.0) → plus de signaux

# Commun
ADX_MIN_C        = 20
TIME_STOP_H      = 72        # 3 jours max
COOLDOWN_BARS    = 8         # 8h entre deux trades même symbole (v21: 12)
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v22 | BB Squeeze Focus | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
...
def run_backtest(cfg_name: str = "Agressif") -> dict:
    ...
    console.print(f"\n[bold]PRISM v22 — BB Squeeze Focus | {cfg_name}[/bold]")
    console.print(
        f"  OKX REST | {MONTHS} mois | {TIMEFRAME} | Capital €{INITIAL_CAPITAL:.0f}\n"
        f"  Pattern C (BB Squeeze) uniquement | SL = ATR14 × {ATR_SL_MULT} | TP = SL × {RR_RATIO:.0f}\n"
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v22 — BB Squeeze Focus | Levier 10-15× | Pattern C Only[/bold yellow]"
    )
    console.print(
        "[bold cyan]  ATR Dynamic SL | Vol ≥ 1.5 | Squeeze 4 barres | Cooldown 8h[/bold cyan]\n"
    )
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison configs — 6 mois | BB Squeeze | Levier 10-15×",
            box=box.ROUNDED
        )
