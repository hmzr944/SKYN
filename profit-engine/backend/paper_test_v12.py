#!/usr/bin/env python3
"""
SKYN v12 — FAST SCALP MODE (1h bars simulant le 15m)
=====================================================
Note : yfinance limite les données 15m à 60 jours max.
On simule le scalping sur barres 1h avec des paramètres très serrés.

Différences vs v11 :
  - sq_bars = 2 (2h squeeze vs 5h) → 3-5× plus de setups
  - VOL_SURGE = 2.5 (vs 3.0) → plus sensible aux poussées de volume
  - ADX_RISE_MIN = 4.0 (vs 3.0) → momentum plus strict pour compenser
  - BO_COOLDOWN_H = 1h (vs 4h) → re-entrée rapide après loss
  - SL = 0.3% (vs 0.5%) → SL serré, cap ATR ×2 max
  - TP scalp : 3F=1.8%, 5F=2.5% → RR 6:1 et 8.3:1
  - PARTIAL_TP désactivé (sort proprement au TP)
  - Macro ±2%, 4F exclu, neutral skip — hérité de v11
"""
...
# ── Paramètres scalp (vs v11) ─────────────────────────────────────────────────
PARTIAL_TP_FRAC   = 0.95  # désactivé en scalp (sort au TP complet)
ADX_RISE_MIN      = 4.0   # plus strict car squeeze plus court
MIN_ADX_BO        = 16
BO_BODY_PCT_MIN   = 0.50
BB_SQUEEZE_WINDOW = 30
BO_COOLDOWN_H     = 1     # re-entrée rapide après loss
COMMISSION        = 0.0004
SLIPPAGE          = 0.0005
...
OI_VOL_LOOKBACK   = 8
VOL_SURGE_MIN     = 2.5   # plus sensible (était 3.0)

# ── 3F et 5F uniquement ───────────────────────────────────────────────────────
ALLOWED_FILTER_COUNTS = {3, 5}

# ── Tiers scalp : TP serré, RR 6:1–8:1 ───────────────────────────────────────
RISK_MULT_BY_F = {5: 2.0, 3: 1.0}
TP_PCT_BY_F    = {5: 0.025, 3: 0.018}   # 2.5% et 1.8%
SL_ATR_MULT    = 0.5      # cap ATR pour SL serré (était 0.8)
SL_MAX_MULT    = 2.0      # SL max = base × 2 (était × 2.4)

# ── 3 Configurations scalp ────────────────────────────────────────────────────
CONFIGS = [
    {"name": "Scalp-Select", "score_min": 88, "sq_bars": 2,
     "risk_pct": 0.040, "sl_pct": 0.003},
    {"name": "Scalp-Opt",    "score_min": 85, "sq_bars": 2,
     "risk_pct": 0.050, "sl_pct": 0.003},
    {"name": "Scalp-Boost",  "score_min": 82, "sq_bars": 2,
     "risk_pct": 0.055, "sl_pct": 0.003},
]
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
    ...
    for ts in common_timestamps:
        ...
        for cand in candidates[:slots]:
            ...
            # SL scalp : très serré, cap ATR réduit (×0.5 au lieu de ×0.8)
            atr_pct = atr_v / price
            eff_sl  = max(sl_pct, min(atr_pct * SL_ATR_MULT, sl_pct * SL_MAX_MULT))

            # TP scalp tiered par filtre
            tp_pct   = TP_PCT_BY_F.get(cand["filters"], 0.018)
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v12 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v12 Scalp[/bold cyan]",
        border_style="cyan",
    ))
    ...
    console.rule(f"[bold]FIN v12 Scalp · {elapsed:.1f}s · {len(w_trades)} trades · SL 0.3% · sq 2h · macro ±2%[/bold]")
...
def run_configs(sym_data, window_timestamps, btc_macro, window_start):
    results = []
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations scalp "
                  f"(sq=2h, SL=0.3%, TP=1.8-2.5%, cd=1h, {N_MAX_POSITIONS} pos)...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v12 — FAST SCALP 1H / SL 0.3% / SQ 2H[/bold yellow]")
    ...
    console.print()
    cmp_table = Table(title="Comparatif v12 Scalp — SL 0.3% / SQ 2h / Cooldown 1h",
