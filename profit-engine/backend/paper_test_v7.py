#!/usr/bin/env python3
"""
SKYN v7 — OPTIMISATION SUR 6 MOIS RÉELS / 500 EUR
====================================================
Objectif : trouver automatiquement les meilleurs parametres pour
les 6 derniers mois de donnees reelles, capital de depart 500 EUR.

Nouveautes vs v6 :
  - Capital 500 EUR
  - Filtre MACRO BTC : alignement avec la tendance BTC daily
    (only long quand BTC bullish, only short quand BTC bearish)
  - SL adaptatif ATR : max(SL_PCT, 0.5 × ATR%) → evite les whipsaws
  - Grid search sur 4 parametres cles (81 combos, ~4 min)
  - Rapport principal sur les 6 DERNIERS MOIS
  - Plus de symboles : 15 actifs (vs 11)

Strategie de base (identique v6) :
  - BB Squeeze 5 couches : Squeeze + Score + Funding + OI + Close
  - Levier 5-10x auto selon score
  - TP 4% fixe, SL adaptatif

Usage :
    cd /home/user/profit-engine/backend
    python paper_test_v7.py
"""
...
SYMBOLS_YF = [
    ...
    "DOGE-USD", "MATIC-USD", "UNI-USD", "NEAR-USD",
]
SYMBOL_MAP = {
    ...
    "DOGE-USD": "DOGE/USDT", "MATIC-USD": "MATIC/USDT",
    "UNI-USD": "UNI/USDT",   "NEAR-USD": "NEAR/USDT",
}
...
# ── Parametres fixes ──────────────────────────────────────────────────────────
TP_PCT           = 0.040   # TP 4% fixe (SL varie selon ATR)
...
# ── 3 configurations à tester (conservateur / équilibré / agressif) ───────────
CONFIGS = [
    {"name": "Conservateur", "score_min": 85, "sq_bars": 7, "risk_pct": 0.040, "sl_pct": 0.005},
    {"name": "Equilibre",    "score_min": 80, "sq_bars": 5, "risk_pct": 0.055, "sl_pct": 0.005},
    {"name": "Agressif",     "score_min": 75, "sq_bars": 3, "risk_pct": 0.070, "sl_pct": 0.006},
]
...
def precompute_symbol(sym: str, cfg) -> Optional[dict]:
    ...
    df["bbw_q15"]  = df["bbw"].rolling(BB_SQUEEZE_WINDOW).quantile(0.15).fillna(df["bbw"])
    ...
    for ii in range(n):
        ...
        rb = int(_e9[ii]>_e21[ii])+int(_e21[ii]>_e50[ii])+int(_e50[ii]>_e200[ii])+int(_p>_e21[ii])
        rs = int(_e9[ii]<_e21[ii])+int(_e21[ii]<_e50[ii])+int(_e50[ii]<_e200[ii])+int(_p<_e21[ii])
        if rb >= 3: _b += 20
        if rs >= 3: _s += 20
...
# ── BTC Macro Regime (filtre directionnel) ────────────────────────────────────

def _build_btc_macro(btc_sd: dict) -> Dict[pd.Timestamp, str]:
    """
    Retourne un dict timestamp → "bull"|"bear"|"neutral"
    base sur ema50 vs ema200 daily de BTC.
    """
...
# ── Breakout detection (adaptatif : prend sq_bars et score_min en param) ──────

def _check_breakout(sd: dict, bar: int, adx_val: float, adx_series: pd.Series,
                    sq_bars: int, score_min: int, vol_surge_min: float = 3.5
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
               ...
               n_max_pos: int = 4) -> Tuple[List[PaperTrade], list, dict]:
    ...
    for ts in common_timestamps:
        ...
        for sym in list(positions.keys()):
            ...
            if reason == "liquidation":
                ...
                if reason == "stop_loss":
                    exit_p = tgt * (1 + SLIPPAGE) if pos["side"] == "long" else tgt * (1 - SLIPPAGE)
                else:
                    exit_p = tgt * (1 - SLIPPAGE) if pos["side"] == "long" else tgt * (1 + SLIPPAGE)
        ...
        for sym, sd in sym_data.items():
            ...
            if nf < 3 or action is None: continue

            # ── FILTRE MACRO BTC ─────────────────────────────────────────────
            if macro == "bull" and action == "SELL": continue
            if macro == "bear" and action == "BUY":  continue
            # En neutral : on laisse passer les deux

            dir_sc = int(sd["buy_sc"][bar]) if action == "BUY" else int(sd["sell_sc"][bar])
            candidates.append({"score": dir_sc, "filters": nf, "sym": sym, "action": action, "bar": bar})
        ...
        for cand in candidates[:slots]:
            ...
            atr_raw = row.get("atr")
            atr_v   = float(atr_raw) if (atr_raw is not None and not pd.isna(atr_raw)) else price * 0.015

            # SL adaptatif : au moins sl_pct, au plus 0.8× ATR%
            atr_pct = atr_v / price
            eff_sl  = max(sl_pct, min(atr_pct * 0.8, 0.012))   # cap à 1.2%
            tp_dist = eff_sl * (TP_PCT / sl_pct)               # maintenir 10:1 RR

            lev      = _get_lev(cand["score"])
            risk_eur = total_eq * risk_pct
...
# ── Metrics rapides (pour grid search) ───────────────────────────────────────
...
def _quick_metrics(trades: List[PaperTrade], equity_curve: list,
    ...
    # Filtrer sur la fenetre des 6 derniers mois
    ...
    return {"net": net, "ret": ret, "wr": wr, "dd": dd, "sharpe": sh, "trades": len(w_trades)}
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    w_eq     = [(ts, v) for ts, v in equity_curve if ts >= window_start]
    if not w_eq:
        console.print("[red]Aucun trade dans la fenetre 6 mois.[/red]")
        return
    ...
    f4_c    = sum(1 for t in w_trades if t.filters == 4)
    f3_c    = sum(1 for t in w_trades if t.filters == 3)

    console.print()
    console.rule("[bold yellow]SKYN v7 — RÉSULTATS 6 MOIS RÉELS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        f"[bold]Trades          [/bold] {len(w_trades)}  (5F:{f5_c} 4F:{f4_c} 3F:{f3_c})\n"
        f"[bold]Levier moyen    [/bold] {avg_lev:.1f}x  |  Liquidations: {liq_c}\n"
        f"[dim]Params: {params_str}[/dim]",
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR[/bold cyan]",
        border_style="cyan",
    ))

    # Explosive days
    ws_naive = window_start.tz_localize(None) if window_start.tzinfo is not None else window_start
    w_day_wins = {d: s for d, s in day_wins.items()
                  if pd.Timestamp(d) >= ws_naive and len(s) >= 2}
    win_counts = [len(s) for d, s in day_wins.items() if pd.Timestamp(d) >= ws_naive]
    max_sim_v2 = max(win_counts) if win_counts else 0
    ...
    console.print(Panel(
        ...
        f"[bold]Max victoires simultanées          :[/bold] {max_sim_v2}",
    ...
    for m in sorted(df_t["month"].unique()):
        ...
        if ret_m >= 100: stat = "[bold green]×2+[/bold green]"
    ...
    # Top trades
    if w_trades:
        top = sorted(w_trades, key=lambda t: t.pnl_eur, reverse=True)[:8]
        tw = Table(title="Top 8 Meilleurs Trades", box=box.SIMPLE)
    ...
    if eq_vals:
        ...
        def tb(v):
            ...
            return chars[min(int((v - eq_min) / (eq_max - eq_min) * 8), 8)]
        console.print(Panel(
    ...
    console.rule(f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades sur 6 mois[/bold]")
...
def run_configs(sym_data, window_timestamps, btc_macro, window_start):
    """
    Teste les 3 configs sur les timestamps 6 mois UNIQUEMENT.
    Rapide : ~20-30s par config au lieu de ~60s sur 2 ans.
    """
    results = []
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations sur 6 mois...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v7 — OPTIMISATION 6 MOIS / 500 EUR[/bold yellow]")

    cfg = AppConfig()
    console.print("Téléchargement et calcul des indicateurs...")
    sym_data = {}
    ...
    console.print(f"  Fenêtre 6 mois : {window_start.strftime('%Y-%m-%d')} → {last_ts.strftime('%Y-%m-%d')}\n")

    # Timestamps uniquement sur la fenêtre 6 mois (rapide)
    ...
    # Tableau comparatif
    console.print()
    cmp_table = Table(title="Comparatif des 3 Configurations", box=box.SIMPLE_HEAVY)
    ...
    console.print(f"\n[bold green]▶ Meilleure config : {best['name']} "
                  ...
                  f"risk={best['risk_pct']*100:.0f}%)[/bold green]")
