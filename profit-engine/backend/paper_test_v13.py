#!/usr/bin/env python3
"""
SKYN v13 — MACRO STREAK ≥2J + 5F TP 7%
========================================
Base : v13 (±2% macro, 3F+5F, neutral skip) → +199.08%

Problème résiduel : pertes Apr-Jun (-18.6%) dues aux trades
sur le 1er jour d'un retour bull/bear après une période neutre.

Deux changements chirurgicaux :
  1. Macro streak ≥ 2 jours : n'entre que si le macro est stable
     depuis au moins 2 jours consécutifs → élimine les "faux départs"
  2. 5F TP 6.5% → 7.0% : capture plus sur les trades haute conviction

Tout le reste identique à v13.
"""
...
TP_PCT_BY_F    = {5: 0.070, 3: 0.045}   # v13: 5F TP 6.5%→7.0%

# ── v13 : stabilité macro requise avant entrée ────────────────────────────────
MACRO_STREAK_MIN = 2   # nombre de jours consécutifs dans le même état
...
def _build_btc_macro(btc_sd: dict) -> Tuple[Dict, Dict]:
    """
    v13 : retourne (macro_state, macro_streak).
    bull   : ema50 > ema200 × 1.02
    bear   : ema50 < ema200 × 0.98
    neutral: tout le reste
    streak : nombre de jours consécutifs dans le même état (pour filtrer
             les "faux départs" le 1er jour d'une nouvelle tendance).
    """
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
               btc_macro: Dict, btc_streak: Dict,
    ...
    for ts in common_timestamps:
        ...
        macro_streak = btc_streak.get(pd.Timestamp(day_key).date(), 0)
        ...
        for sym, sd in sym_data.items():
            ...
            # v13 : streak ≥ 2j pour les 3F seulement (les 5F ont conviction propre)
            if nf == 3 and macro_streak < MACRO_STREAK_MIN:
                continue
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v13 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v13[/bold cyan]",
        border_style="cyan",
    ))
    ...
    console.rule(f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades · 4F exclus · macro ±2% streak≥{MACRO_STREAK_MIN}j · 5F TP 7%[/bold]")


# ── Test 3 configurations ─────────────────────────────────────────────────────

def run_configs(sym_data, window_timestamps, btc_macro, btc_streak, window_start):
    results = []
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"(3F+5F, macro ±2%, streak≥{MACRO_STREAK_MIN}j, {N_MAX_POSITIONS} pos)...[/bold cyan]")
    ...
    for cfg in CONFIGS:
        ...
        trades, eq_curve, dw = run_engine(
            sym_data, window_timestamps, btc_macro, btc_streak,
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v13 — MACRO ±2% STREAK≥2J / 3F+5F / 5F-TP-7%[/bold yellow]")
    ...
    btc_macro, btc_streak = _build_btc_macro(sym_data["BTC-USD"])
    ...
    results = run_configs(sym_data, window_timestamps, btc_macro, btc_streak, window_start)

    console.print()
    cmp_table = Table(title="Comparatif v13 — 3 Configurations (macro ±2% streak≥2j, 4F exclu, 5F-TP-7%)",
