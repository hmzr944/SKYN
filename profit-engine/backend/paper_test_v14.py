#!/usr/bin/env python3
"""
SKYN v14 — MOMENTUM BTC 4H + 5F TP 7%
=======================================
Base : v11 (±2% macro, 3F+5F, neutral skip) → +184.82% actuel

Problème résiduel : Apr-Jun 4 trades shorts perdants (-284€) entrés
pendant des rebonds BTC au sein d'une macro bear (BTC remontait à
court terme le jour de l'entrée).

Deux changements :
  1. BTC 4h momentum check : avant toute entrée SHORT (macro bear),
     vérifier que BTC close actuel < BTC close il y a 4 barres × 1.001
     → on ne short que si BTC continue de baisser, pas lors des rebonds
     Inverse pour LONG (macro bull).
  2. 5F TP 6.5% → 7.0% : en réserve pour quand les 5F réapparaissent.
...
TP_PCT_BY_F    = {5: 0.070, 3: 0.045}   # v14: 5F TP 7%

# ── v14 : filtre momentum BTC court-terme ─────────────────────────────────────
BTC_MOM_BARS   = 4      # lookback en barres 1h
BTC_MOM_THRESH = 0.001  # 0.1% — BTC doit baisser d'au moins 0.1% sur 4h pour un SHORT

# ── v14 : filtre RSI daily BTC — évite les shorts en recovery ─────────────────
BTC_RSI_BEAR_MAX = 55   # bear macro: RSI daily > 55 = recovery en cours → skip short
BTC_RSI_BULL_MIN = 45   # bull macro: RSI daily < 45 = correction → skip long

# ── v14 : filtre "marge restante" — ne short pas au fond ─────────────────────
BTC_DOWNSIDE_MIN = 0.05   # SHORT seulement si BTC > 30j-low × 1.05 (5% au-dessus du support)
BTC_UPSIDE_MIN   = 0.05   # LONG seulement si BTC < 30j-high × 0.95 (5% sous la résistance)
...
def resample_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["low30d"]  = c.rolling(30, min_periods=10).min()   # support 30j
    d["high30d"] = c.rolling(30, min_periods=10).max()   # résistance 30j
    return d
...
def _build_btc_macro(btc_sd: dict) -> Tuple[Dict, Dict, Dict, Dict]:
    """
    v14 : retourne (macro_state, daily_rsi, btc_low30, btc_close_daily).
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
    ...
    for ts in common_timestamps:
        ...
        btc_low30_day = btc_low30.get(pd.Timestamp(day_key).date(), 0.0)
        btc_hi30_day  = btc_high30.get(pd.Timestamp(day_key).date(), 0.0)
        ...
        # v14 : RSI daily BTC — évite les shorts en recovery / longs en correction
        if macro == "bear" and btc_rsi_day > BTC_RSI_BEAR_MAX:
            continue   # BTC RSI > 55 en bear = recovery → skip shorts
        if macro == "bull" and btc_rsi_day < BTC_RSI_BULL_MIN:
            continue   # BTC RSI < 45 en bull = correction → skip longs

        # v14 : BTC "marge restante" — ne short pas si BTC est déjà au fond
        if macro == "bear" and btc_low30_day > 0:
            btc_sd_tmp = sym_data.get("BTC-USD")
            btc_bar_tmp = btc_sd_tmp["ts_to_pos"].get(ts) if btc_sd_tmp else None
            if btc_bar_tmp is not None:
                btc_close_now = float(btc_sd_tmp["df"].iloc[btc_bar_tmp]["close"])
                if btc_low30_day > 0 and btc_close_now < btc_low30_day * (1 + BTC_DOWNSIDE_MIN):
                    continue   # BTC trop proche de son support 30j → skip short
        if macro == "bull" and btc_hi30_day > 0:
            btc_sd_tmp = sym_data.get("BTC-USD")
            btc_bar_tmp = btc_sd_tmp["ts_to_pos"].get(ts) if btc_sd_tmp else None
            if btc_bar_tmp is not None:
                btc_close_now = float(btc_sd_tmp["df"].iloc[btc_bar_tmp]["close"])
                if btc_hi30_day > 0 and btc_close_now > btc_hi30_day * (1 - BTC_UPSIDE_MIN):
                    continue   # BTC trop proche de sa résistance 30j → skip long

        # v14 : BTC 4h momentum — ne trade que si BTC va dans le sens du macro
        ...
        if btc_sd is not None and btc_bar is not None and btc_bar >= BTC_MOM_BARS:
            ...
            if btc_prev > 0:
                ...
                if macro == "bear" and btc_chg > BTC_MOM_THRESH:
                    continue   # BTC remonte — pas de short pendant rebond
                if macro == "bull" and btc_chg < -BTC_MOM_THRESH:
                    continue   # BTC baisse — pas de long pendant recul
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v14 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v14[/bold cyan]",
        border_style="cyan",
    ))
    ...
    console.rule(f"[bold]FIN · {elapsed:.1f}s · {len(w_trades)} trades · 4F excl · macro±2% · BTC-mom-4h · 5F-TP-7%[/bold]")
...
def run_configs(sym_data, window_timestamps, btc_macro, btc_rsi, btc_low30, btc_high30, window_start):
    ...
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"(3F+5F, macro±2%, RSI-daily, BTC-mom-4h, {N_MAX_POSITIONS} pos)...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v14 — MACRO ±2% / BTC 4H MOMENTUM / 5F TP 7%[/bold yellow]")
    ...
    console.print()
    cmp_table = Table(title="Comparatif v14 — 3 Configurations (macro±2%, BTC-mom-4h, 4F exclu)",
