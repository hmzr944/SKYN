#!/usr/bin/env python3
"""
SKYN v8 — GAIN EXPLOSIF / HAUTE FRÉQUENCE FILTRÉE
==================================================
Objectif : +200% en 1-2 mois avec 500 EUR de capital.

Nouveautés vs v7 :
  - 18 symboles (TRX, ALGO, FIL, APT, INJ ajoutés)
  - N_MAX_POSITIONS = 5 (vs 4)
  - VOL_SURGE_MIN = 3.0 (vs 3.5) — plus d'opportunités dans la direction macro
  - Risque et TP TIERED par nombre de filtres validés :
      5F → Risk×1.75  TP 5.5%  (haute conviction)
      4F → Risk×1.25  TP 4.8%  (bonne conviction)
      3F → Risk×1.0   TP 4.0%  (conviction standard)
  - Bonus macro confirmé : TP +20% quand direction = macro BTC
  - Macro neutral renforcé : score+5 requis vs configs standard
  - 3 configs offensives : Selectif / Optimal / Boost

Usage :
    cd /home/user/profit-engine/backend
    python paper_test_v8.py
"""
...
SYMBOLS_YF = [
    ...
    "DOGE-USD", "NEAR-USD",
    "TRX-USD", "ALGO-USD", "FIL-USD", "APT-USD", "INJ-USD",
]
...
OI_VOL_LOOKBACK   = 8
VOL_SURGE_MIN     = 3.0   # v8 : 3.0 (vs 3.5 en v7) — plus d'opportunités

# ── Tier par nombre de filtres ────────────────────────────────────────────────
RISK_MULT_BY_F = {5: 1.75, 4: 1.25, 3: 1.0}
TP_PCT_BY_F    = {5: 0.055, 4: 0.048, 3: 0.040}
MACRO_TP_BONUS = 1.20   # +20% TP quand direction alignée avec macro BTC
...
CONFIGS = [
    ...
    {"name": "Boost",    "score_min": 80, "sq_bars": 3,
     "risk_pct": 0.055, "sl_pct": 0.006},
]
...
def run_engine(sym_data: dict, common_timestamps: List[pd.Timestamp],
    ...
    for ts in common_timestamps:
        ...
        for sym, sd in sym_data.items():
            ...
            # Macro neutral : score plus élevé requis
            if macro == "neutral" and dir_sc < score_min + 5:
                continue

            candidates.append({"score": dir_sc, "filters": nf, "sym": sym,
                                "action": action, "bar": bar, "macro": macro})
        ...
        for cand in candidates[:slots]:
            ...
            # TP tiered par filtre + bonus macro
            tp_base = TP_PCT_BY_F.get(cand["filters"], 0.040)
            macro_bonus = MACRO_TP_BONUS if cand["macro"] in ("bull", "bear") else 1.0
            tp_dist = eff_sl * (tp_base / sl_pct) * macro_bonus

            # Risque tiered par filtre
...
def print_report(trades, equity_curve, day_wins, window_start, params, t0):
    ...
    console.print()
    console.rule("[bold yellow]SKYN v8 — RÉSULTATS 6 MOIS / 500 EUR[/bold yellow]")
    console.print()
    ...
    console.print(Panel(
        ...
        title="[bold cyan]6 Derniers Mois · Capital 500 EUR · v8[/bold cyan]",
        border_style="cyan",
    ))

    # Tier breakdown
    for nf, label in [(5, "5F — Haute conviction"), (4, "4F — Bonne conviction"), (3, "3F — Standard")]:
...
def run_configs(sym_data, window_timestamps, btc_macro, window_start):
    ...
    console.print(f"\n[bold cyan]Test de {len(CONFIGS)} configurations "
                  f"({N_MAX_POSITIONS} positions max, vol≥{VOL_SURGE_MIN})...[/bold cyan]")
...
def main():
    t0 = time.time()
    console.rule("[bold yellow]SKYN v8 — GAIN EXPLOSIF / HAUTE FRÉQUENCE FILTRÉE[/bold yellow]")
    ...
    console.print()
    cmp_table = Table(title="Comparatif v8 — 3 Configurations", box=box.SIMPLE_HEAVY)
