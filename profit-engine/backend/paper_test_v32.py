#!/usr/bin/env python3
"""
PRISM v32 — Exclusion FLOKI-USDT | Suppression actif toxique
=============================================================
Base : v31 (24 symboles, two-pass score-priority, margin cap 60%, score_min 70/65/60)
Delta v32 vs v31 :
  - Suppression FLOKI-USDT → 23 symboles
  - FLOKI : meme coin, 0% WR sur 2-3 trades selon config
    Optimal : -132€ | Agressif : -268€
  - Comportement BB Squeeze invalide sur meme coins (IVOL extrême,
    wash trading, pas de mean-reversion structurelle post-breakout)
  - Logique identique aux exclusions précédentes : FIL, SOL, APT, WLD

Scores identiques à v31 (70/65/60) — le relèvement n'avait pas d'effet
car tous les signaux avaient déjà score >= 65. On garde ces seuils.
"""
...
SYMBOLS = [
    # v32 : 23 symboles (v31 moins FLOKI-USDT 0%WR, -132€ Optimal)
    "BTC-USDT", "AVAX-USDT",
...
ADX_MIN_C        = 20
TIME_STOP_H      = 72
COOLDOWN_BARS    = 5       # v22: 8 → v30: 5 (plus de signaux)
...
CONFIGS = [
    {"name": "Prudent",   "risk_pct": 0.06, "max_pos": 2, "score_min": 70},
    {"name": "Optimal",   "risk_pct": 0.10, "max_pos": 4, "score_min": 65},
    {"name": "Agressif",  "risk_pct": 0.14, "max_pos": 5, "score_min": 60},
]
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v32 | 23 Symboles | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
    ...
    if trades:
        ...
        stbl = Table(title="PnL par Symbole (v32 — 23 symboles)", box=box.SIMPLE_HEAD)
...
def run_backtest(cfg_name: str = "Agressif") -> dict:
    ...
    console.print(f"\n[bold]PRISM v32 — 23 Symboles | {cfg_name}[/bold]")
    console.print(
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v32 — Exclusion FLOKI | 23 Symboles | Cooldown 5h[/bold yellow]"
    )
    console.print(
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison configs — 6 mois | 23 Symboles | Cooldown 5h",
            box=box.ROUNDED
        )
