#!/usr/bin/env python3
"""
PRISM v26 — Scale-Out | 30 Symbols | More Trades Per Month
===========================================================
Base : v22 (BB Squeeze Focus, Pattern C only, ADX/score leverage)
Delta v26 vs v22 :
  - 30 symboles  (17 → 30, +ARB APT SUI UNI AAVE WLD TIA SEI HBAR ICP STX FLOKI JUP)
  - COOLDOWN_BARS : 8 → 5  (plus de signaux par symbole)
  - max_pos Agressif : 4 → 5
  - score_min Agressif : 60 → 55
  - BACKTEST_START fixe 2026-01-01 (fenêtre stable indépendante du cache)
  - Rapport par symbole pour identifier les symboles à retirer

Objectif : 16+ trades/mois vs 8 en v22 → +20% bons mois, -4% mauvais mois.
"""
...
SYMBOLS = [
    ...
    # Nouveaux v26 — 13 symboles vérifiés disponibles sur OKX
...
ADX_MIN_C        = 20
TIME_STOP_H      = 72
COOLDOWN_BARS    = 5       # v22: 8 → v26: 5 (plus de signaux)
...
def print_report(trades, eq_curve, cfg_name: str, t0: float):
    ...
    console.print(f"\n[bold cyan]━━━ PRISM v26 | 30 Symboles | Config: {cfg_name} ━━━[/bold cyan]")
    console.print(
    ...
    if trades:
        ...
        stbl = Table(title="PnL par Symbole (v26 — 30 symboles)", box=box.SIMPLE_HEAD)
...
def run_backtest(cfg_name: str = "Agressif") -> dict:
    ...
    console.print(f"\n[bold]PRISM v26 — 30 Symboles | {cfg_name}[/bold]")
    console.print(
        ...
        f"  Symboles: {len(SYMBOLS)} (17 v22 + 13 nouveaux)"
    )
...
if __name__ == "__main__":
    console.print(
        "[bold yellow]PRISM v26 — Scale-Out | 30 Symboles | Cooldown 5h[/bold yellow]"
    )
    console.print(
    ...
    if results:
        console.print("\n")
        tbl = Table(
            title="Comparaison configs — 6 mois | 30 Symboles | Cooldown 5h",
            box=box.ROUNDED
        )
