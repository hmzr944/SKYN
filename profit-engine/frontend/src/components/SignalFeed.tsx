        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-xs font-bold px-2 py-0.5 rounded border ${ACTION_STYLE[sig.action]}`}>
                {sig.action}
              </span>
              <span className={`text-xs font-mono ${CONF_COLOR[sig.confidence]}`}>
                {sig.score.toFixed(0)}%
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-between text-xs text-gray-400 mb-2">
            ...
            <span className={CONF_COLOR[sig.confidence]}>{sig.confidence}</span>
          </div>
